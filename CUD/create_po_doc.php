	...
	$sql7 = "select * from document_mm  where doc_mm_id='$po_number'";
		...
		if($rows7>$i){
			...
			if(mysqli_query($connect,$sql))
			{
				// header('location: http://localhost/admin2/sd_inquery.php');
				 echo "<script>
					alert('Purchase order $po_number was updated.');
					window.location.href='http://localhost/admin2/mm_po_next.php';
					</script>";
					// 
			}
			else
		...
		else
		{
			...
			if(mysqli_query($connect,$sql))
			{
				// header('location: http://localhost/admin2/sd_inquery.php');
				 echo "<script>
					alert('Purchase order $po_number was created.');
					window.location.href='http://localhost/admin2/mm_before_po.php';
					</script>";
			}
			else
			{
