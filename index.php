...
include("includes/connection.php");
								...
								$sql = "SELECT doc_date,description,material_number,quantity,SUM(price) as price
										from document join inq_has_mat using (doc_id) 
													  join material using (material_number) GROUP BY doc_date,description; ";
								$result=mysqli_query($connect,$sql); //rs.open sql,con

								$sql2 = "SELECT sum(price*quantity) as total
											FROM (document t1  LEFT OUTER JOIN document t2  ON t1.doc_id = t2.ref_doc) 
											join inq_has_mat on t1.doc_id = inq_has_mat.doc_id
											join material using (material_number)
											where t2.doc_id like 'INV%'
			            ...
			            $sql6 = "SELECT ref_doc from document where doc_type_id = 5";
								$result6=mysqli_query($connect,$sql6);//rs.open sql,con
                         $row_num = 0 ;
 						...
 						for($x=0; $x<$row_num;$x++ ){
							...
							$sql4 = "select doc_id,sum(cost_price*quantity) as sum 
									from document join inq_has_mat using 					
									(doc_id) join material using (material_number) where doc_id = $ref_doc[$x]";
								$result4=mysqli_query($connect,$sql4);	
							while ($row=mysqli_fetch_array($result4))		{
							 $total_sum_in = $total_sum_in+$row['sum'];
								}	
						}

							
							$sql7 = "select sum(net_price) as total from document where doc_type_id = 5 ;";
            ...
            <div class="card-body">
              <div class="row">
                ...
                <div class="text-center ">
					<br>
                  <div class="text-primary"><?php $pofit = 0; while ($row=mysqli_fetch_array($result7))	{
												$pofit = $row['total'];
								echo "$";	echo number_format($row['total'],2) ;} ?> </div>
                  <div class="small text-muted">ยอดขายทั้งหมดที่เกิดขึ้น</div>
                  <hr>
                  <div class=" text-warning"><?php $pofit = $pofit-$total_sum_in; echo "$"; echo number_format($pofit,2) ;?></div>
