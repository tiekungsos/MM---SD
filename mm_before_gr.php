...
<script language="javascript">
	...
	if(strPage == "page1")
	{
	document.form1.action="mm_gr_display.php";
	}
	if(strPage == "page2")
	{
	document.form1.action="mm_gr_display.php";
...
<!--		<button type="button" class="btn btn-primary" id="Display" value="2">Create</button>   -->
  ...
  <button type="button" class="btn btn-primary" id="Create" value="3">Display</button>   -->   
	...
	<div class="box span12" style="margin-left: 0px;">
					<form action="mm_gr_next.php" method="post">
					...
					<div class="box-content"> 
						...
						<div class="box span12" style="margin-left: 0px;">
					<div class="box-header" data-original-title;>
						<h2><i class="icon-file white user"></i><span class="break"></span>Ref. Purchase order</h2>
					</div>
					<div class="box-content"  	> 
						<label for="usr">
							<label for="usr">Purchase order number : 
								<select name="rfq_number" onchange="showCustomer(this.value)">
								<?php
									include("includes/connection.php");

									$sql2 = "SELECT * FROM document_mm WHERE vendor_id = '$vendor_id'&& doc_mm_type_id=2 ";
									$result2=mysqli_query($connect,$sql2); //rs.open sql,con

								while ($row=mysqli_fetch_array($result2))
								{ ?><!--open of while -->
									<option value="<?= $row['ref_mm_doc'] ?>"><?= $row['doc_mm_id'] ?></option>
								<?php } ?>
								</select>
							</label>
							<p></p>
				...
				<form action="mm_gr_display.php" method="post">
						<div style="display: none">
						...
						<div class="box span12" style="margin-left: 0px;">
					<div class="box-header" data-original-title;>
						<h2><i class="icon-file white user"></i><span class="break"></span>Display Goods Receipt</h2>
						<div class="box-icon">
							<a href="#" class="btn-setting"><i class="halflings-icon white wrench"></i></a>
							<a href="#" class="btn-minimize"><i class="halflings-icon white chevron-up"></i></a>
							<a href="#" class="btn-close"><i class="halflings-icon white remove"></i></a>
					...
					<div class="box-content"> 
						 ...
						 <div class="form-group">
							...
							<div class="form-group">
  								<label for="sel1">Goods Receipt List:</label>
  									<select name="rfq_number" onchange="showCustomer(this.value)">
								<?php
									include("includes/connection.php");

									$sql2 = "SELECT * FROM document_mm WHERE vendor_id = '$vendor_id'&& doc_mm_type_id=3 ";
									$result2=mysqli_query($connect,$sql2); //rs.open sql,con
		  ...
		  <table class="table table-striped table-bordered bootstrap-datatable datatable">
						  ...
						  <tbody>
							...
							<tr>
								...
								<td class="center">
									<a class="btn glyphicon glyphicon-ok" id="add_customer" href="mm_before_gr.php?id=<?= $row['vendor_id'] ?>" >
